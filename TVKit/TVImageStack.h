//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSArray, NSMutableSet, NSString;
@protocol OS_dispatch_queue;

@interface TVImageStack : NSObject
{
    int _blendMode;
    double _flatImageCornerRadius;
    NSArray *_imageRepresentations;
    NSMutableSet *_loadingOperations;
    struct CGSize _naturalSize;
    NSObject<OS_dispatch_queue> *_queue;
    NSString *_temporaryFilesystemPath;
    _Bool _topLayerIsFixedFrame;
    id _placeholderImage;
    id _flatImage;
}

+ (struct CGSize)_naturalSizeForImageStackSpecification:(id)arg1;
+ (id)_loadingOperationQueue;
+ (id)_imageRepresentationsForImageStackSpecification:(id)arg1 fromURL:(id)arg2;
+ (id)_imageRepresentationsForUIImage:(id)arg1 outFlatImage:(out id *)arg2 outFlatImageCornerRadius:(out double *)arg3;
+ (id)imageStackWithBaseImageName:(id)arg1;
@property(retain, nonatomic) id flatImage; // @synthesize flatImage=_flatImage;
@property(retain, nonatomic) id placeholderImage; // @synthesize placeholderImage=_placeholderImage;
- (void).cxx_destruct;
- (void)_loadImagesAtSize:(struct CGSize)arg1 scaledSize:(struct CGSize)arg2 completion:(CDUnknownBlockType)arg3;
- (void)loadImagesAtSize:(struct CGSize)arg1 completion:(CDUnknownBlockType)arg2;
- (void)setTopLayerIsFixedFrame:(_Bool)arg1;
- (_Bool)topLayerIsFixedFrame;
- (void)setBlendMode:(int)arg1;
- (int)blendMode;
- (void)setFlatImageCornerRadius:(double)arg1;
- (struct CGSize)naturalSize;
- (double)flatImageCornerRadius;
- (void)cancel;
- (void)dealloc;
- (id)initWithArchivedLayeredImageProxy:(id)arg1;
- (id)initWithDirectoryPath:(id)arg1;
- (id)initWithURL:(id)arg1;
- (id)initWithImageProxies:(id)arg1;
- (id)initWithImages:(id)arg1;
- (id)initWithImageURLs:(id)arg1;
- (id)initWithBaseImageName:(id)arg1 bundle:(id)arg2;
- (id)initWithBaseImageName:(id)arg1;
- (id)init;

@end

